...
public class EntityFunnyDrive
extends Entity implements IThrowableEntity {
    /**
     * ★撃った人
     */
     ...
     * @param par1World
     */
    public EntityFunnyDrive(World par1World)
    {
        super(par1World);
    }

    public EntityFunnyDrive(World par1World, EntityLivingBase entityLiving,float AttackLevel,boolean multiHit, float roll){
        this(par1World,entityLiving,AttackLevel,multiHit);
        this.setRoll(roll);
    }

    public EntityFunnyDrive(World par1World, EntityLivingBase entityLiving,float AttackLevel,boolean multiHit){
        this(par1World, entityLiving, AttackLevel);
        this.setIsMultiHit(multiHit);
    }

    public EntityFunnyDrive(World par1World, EntityLivingBase entityLiving,float AttackLevel)
    {
        this(par1World);
    ...
    private static final DataParameter<Float> ROLL = EntityDataManager.<Float>createKey(EntityFunnyDrive.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> LIFETIME = EntityDataManager.<Integer>createKey(EntityFunnyDrive.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> IS_MULTI_HIT = EntityDataManager.<Boolean>createKey(EntityFunnyDrive.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> IS_SLASH_DIMENSION = EntityDataManager.<Boolean>createKey(EntityFunnyDrive.class, DataSerializers.BOOLEAN);
                ...
                if(!getIsMultiHit() || this.ticksExisted % 2 == 0){
                    ...
                    for(Entity curEntity : list){
                        ...
                        if(!blade.isEmpty() && curEntity instanceof EntityLivingBase) {
                            if(blade.getItem() instanceof ItemFunnySlashBlade){
                                ((ItemFunnySlashBlade)blade.getItem()).hitEntity2(blade, (EntityLivingBase) curEntity, (EntityLivingBase) thrower);
                            }else {
                                ((ItemSlashBlade) blade.getItem()).hitEntity(blade, (EntityLivingBase) curEntity, (EntityLivingBase) thrower);
                            }
                            EntityLivingBase entityLivingBase = (EntityLivingBase)curEntity;
                                ItemStack manHand = entityLivingBase.getHeldItemMainhand();
                                if(!manHand.isEmpty()){
                                    //world.spawnEntity(new EntityItem(world,entityLivingBase.posX,entityLivingBase.posY,entityLivingBase.posZ,manHand.copy()));
                                    entityLivingBase.setHeldItem(EnumHand.MAIN_HAND,ItemStack.EMPTY);
                                }
                                ItemStack offHand = entityLivingBase.getHeldItemOffhand();
                                if(!offHand.isEmpty()){
                                    //world.spawnEntity(new EntityItem(world,entityLivingBase.posX,entityLivingBase.posY,entityLivingBase.posZ,offHand.copy()));
                                    entityLivingBase.setHeldItem(EnumHand.OFF_HAND, ItemStack.EMPTY);
                                }
